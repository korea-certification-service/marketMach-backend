let PointTrades = require('../model/pointTrades');
...
function add(country, data) {
    return new Promise((resolve, reject) => {
        db.connectDB(country)
        .then(() => {
            var pointTrades = new PointTrades(data)
            pointTrades.save(function (err, result) {
                if (err) {
                    reject(err);
                } else {
                    resolve(result);
