let CoinWithdraw = require('../model/coinwithdraw');
...
function add(country, data) {
    return new Promise((resolve, reject) => {
        setTimeout(_ => {
            db.connectDB(country)
            .then(() => {
                var coinWithdraw = new CoinWithdraw(data)
                coinWithdraw.save(function (err, result) {
                    if (err) {
                        reject(err);
                    } else {
                        resolve(result);
